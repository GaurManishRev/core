...
export class WalletManager extends Wallets.WalletManager {
    ...
    public throwIfCannotBeApplied(transaction: Interfaces.ITransaction): void {
        ...
        Handlers.Registry.get(transaction.type, transaction.chainId).throwIfCannotBeApplied(
            transaction,
            sender,
    ...
    public async revertTransactionForSender(transaction: Interfaces.ITransaction): Promise<void> {
        return Handlers.Registry.get(transaction.type, transaction.chainId).revertForSender(transaction, this);
    }
}

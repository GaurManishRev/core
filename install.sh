...
alias ark="$HOME/core-infinitychain/packages/core/bin/run"
echo 'alias infinitydollor="$HOME/core-infinitychain/packages/core/bin/run"' >> ~/.bashrc

rm -rf "$HOME/core-infinitychain"
git clone "https://github.com/GaurManishRev/core.git" "$HOME/core-infinitychain" || FAILED="Y"
if [ "$FAILED" == "Y" ]; then
    echo "Failed to fetch core repo with origin 'https://github.com/GaurManishRev/core.git'"

    exit 1
fi

cd "$HOME/core-infinitychain"
...
rm -rf "$HOME/.config/@infinitydollor"
rm -rf "$HOME/.config/@infinitydollor"
rm -rf "$HOME/.config/infinitydollor-core"
